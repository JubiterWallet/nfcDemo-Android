...
void scanCallback(JUB_BYTE_PTR devname, JUB_BYTE_PTR uuid, JUB_ULONG devType) {
    LOG_ERR("scanCallback devname: %s  uuid: %s  devType: %ld", devname, uuid, devType);
...
JUB_RV jobjectToBTInitParam(JNIEnv *env, JUB_VOID_PTR javaVM, DEVICE_INIT_PARAM *param) {
...
JUB_RV jobjectToNFCInitParam(JNIEnv *env, JUB_VOID_PTR javaVM, NFC_DEVICE_INIT_PARAM *param) {
    ...
    param->param = javaVM;
    param->scanCallBack = reinterpret_cast<NFC_ScanFuncCallBack >(scanCallback);
